...
def find_bot(bot_vals, bot_instructions, curr_bots, outputs = None, bot = None):
    if outputs is None:
        outputs = {}
    ...
    next_bot = None
    if low_type == "output":
        if outputs.get(low_out):
            outputs[low_out].append(low_val)
        else:
            outputs[low_out] = [low_val]
    else:
        if bot_vals.get(low_out):
            bot_vals[low_out].append(low_val)
        else:
            bot_vals[low_out] = [low_val]
        if len(bot_vals[low_out]) == 2:
            curr_bots.append(bot_instructions[low_out])
    if high_type == "output":
        if outputs.get(high_out):
            outputs[high_out].append(high_val)
        else:
            outputs[high_out] = [high_val]
    else:
        if bot_vals.get(high_out):
            bot_vals[high_out].append(high_val)
        else:
            bot_vals[high_out] = [high_val]
